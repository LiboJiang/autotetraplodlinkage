...
ogdata.mapdistance=xlsread('d:\matlab\work\autotetraploidQTL\data\group_part_marker.xls','map');
ogdata.mapdistance(length(ogdata.mapdistance(:,1))+1,:)=NaN;
ogdata.marker=xlsread('d:\matlab\work\autotetraploidQTL\data\group_part_marker.xls','marker');
[~,ogdata.markername,~]=xlsread('d:\matlab\work\autotetraploidQTL\data\group_part_marker.xls','name');
ogdata.phenotype=xlsread('d:\matlab\work\autotetraploidQTL\data\QTLphenotype.xlsx','greenweight');
...
chr=0;
tempsum_d=0;
for i=1:length(ogdata.mapdistance(1,:))
    j=1;
    while ~isnan(ogdata.mapdistance(j,i))
        tempsum_d=tempsum_d+ogdata.mapdistance(j,i);
        chr=[chr,tempsum_d];
        j=j+1;
    end
    chr=[chr,tempsum_d];
...
Lphase(1).case=11;
Lphase(1).dms=[2,2];
Lphase(1).configration=[1,0,0,0;1,0,0,0];
Lphase(1).mtc{1,1}=[1,5,6,7];         Lphase(1).mtc{1,2}=[2,3,4,8,9,10];  
Lphase(1).mtc{2,1}=[1,5,6,7];         Lphase(1).mtc{2,2}=[2,3,4,8,9,10]; 
%%% case ss2 
%%% 1 0 0 0
%%% 0 1 0 0
Lphase(2).case=12;
Lphase(2).dms=[2,2];
Lphase(2).configration=[1,0,0,0;0,1,0,0];
Lphase(2).mtc{1,1}=[1,5,6,7];         Lphase(2).mtc{1,2}=[2,3,4,8,9,10];  
Lphase(2).mtc{2,1}=[2,5,8,9];         Lphase(2).mtc{2,2}=[1,3,4,6,7,10]; 
...
% Lphase(3).case=21;
% Lphase(3).dms=[2,2];
% Lphase(3).configration=[1,1,0,0;1,1,0,0];
% Lphase(3).mtc{1,1}=[1,2,5,6,7,8,9];         Lphase(3).mtc{1,2}=[3,4,10];  
% Lphase(3).mtc{2,1}=[1,2,5,6,7,8,9];         Lphase(3).mtc{2,2}=[3,4,10];  
% %%% case dd2 
% %%% 1 1 0 0
% %%% 1 0 1 0
% Lphase(4).case=22;
% Lphase(4).dms=[2,2];
% Lphase(4).configration=[1,1,0,0;1,0,1,0];
% Lphase(4).mtc{1,1}=[1,2,5,6,7,8,9];         Lphase(4).mtc{1,2}=[3,4,10];  
% Lphase(4).mtc{2,1}=[1,3,5,6,7,8,10];         Lphase(4).mtc{2,2}=[2,4,9];  
% %%% case dd3 
% %%% 1 1 0 0
% %%% 0 0 1 1
% Lphase(5).case=23;
% Lphase(5).dms=[2,2];
% Lphase(5).configration=[1,1,0,0;0,0,1,1];
% Lphase(5).mtc{1,1}=[1,2,5,6,7,8,9];         Lphase(5).mtc{1,2}=[3,4,10];  
% Lphase(5).mtc{2,1}=[3,4,6,7,8,9,10];         Lphase(5).mtc{2,2}=[1,2,5]; 
% %%% case tt1 
% %%% 1 1 1 0
% %%% 1 1 1 0 
% Lphase(6).case=31;
% Lphase(6).dms=[2,2];
% Lphase(6).configration=[1,1,1,0;1,1,1,0];
% Lphase(6).mtc{1,1}=[1,2,3,5,6,7,8,9,10];         Lphase(6).mtc{1,2}=4;  
% Lphase(6).mtc{2,1}=[1,2,3,5,6,7,8,9,10];         Lphase(6).mtc{2,2}=4; 
% %%% case tt2 
% %%% 1 1 1 0
% %%% 1 1 0 1 
% Lphase(7).case=32;
% Lphase(7).dms=[2,2];
% Lphase(7).configration=[1,1,1,0;1,1,0,1];
% Lphase(7).mtc{1,1}=[1,2,3,5,6,7,8,9,10];         Lphase(7).mtc{1,2}=4;  
% Lphase(7).mtc{2,1}=[1,2,4,5,6,7,8,9,10];         Lphase(7).mtc{2,2}=3; 


save('SQdata','Lphase','MQMmtrx','Mmtrx','mkindx','ogdata','scan','coefx');

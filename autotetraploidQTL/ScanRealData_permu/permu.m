...
result=[];
ogdata.phenotype=ogdata.phenotype(randperm(length(ogdata.phenotype)),:);
...
while 1
    ...
    for i=1:30
       ...
       rg=rg/sum(rg);
       tempg1=rand(59,1);
       tempg1=fmincon(@rand59g,tempg1,[],[],Aeq,Beq,zeros(1,59),ones(1,59),[],optionsSD);
       if sum(abs(Aeq*tempg1-Beq))<tempgC
           tempgC=sum(abs(Aeq*tempg1-Beq));
           tempg=tempg1;
       end
    end
    clear tempgC tempg1
...
%         fprintf('Scan %.1f  CRtime=%d--U:(%6.3f  %6.3f  %6.3f)    Sigma2=%6.3f   #emtime=%d\n',scan.point,CRtime,eval.u,eval.sigma2,emtime)
    ...
    fprintf('Permu:%.3d Scan %.1f  LR=%.2f     U:(%6.3f  %6.3f  %6.3f)    Sigma2=%6.3f  Phase=%d  err=%6.3f\n',permu,scan.point,LR,eval.u,eval.sigma2,eval.LinkagePhase,sum(abs(Aeq*eval.g-Beq))) 
  ...
  fprintf('Scan %.1f    Max LR=%4.2f     U:%6.3f  %6.3f  %6.3f    Sigma2=%6.3f     Phase=%d    err=%6.3f\n\n',result1([1,2,5:7,9,14,3]));
  
  if result1(3,1)>16
      errcount=errcount+1;
      fprintf('\nAn Error exceed  err=%f    count=%d\n', result1(3,1),errcount);
  end

  permuresult(:,permu)=result1(:,1);
  save('RealData_permu','permuresult');
  permu=permu+1;
  
  clear result1 result

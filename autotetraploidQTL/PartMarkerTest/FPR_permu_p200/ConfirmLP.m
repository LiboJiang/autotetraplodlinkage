...
function RetureValue = ConfirmLP(mk)

global Mmtrx samp Lphase samplesize scan SM

PHcase=mk-1;

for mk1=1:Lphase(mk-1).dms(1)
    for mk2=1:Lphase(mk-1).dms(2)
        MtrxPaiP{mk1,mk2}=[];
        MtrxCoef{mk1,mk2}=[];
...
%%%%%%%%%
samp.freq=zeros(Lphase(mk-1).dms(1),Lphase(mk-1).dms(2));
for mk1=1:Lphase(mk-1).dms(1)
    for mk2=1:Lphase(mk-1).dms(2)
        for i1=1:length(Lphase(PHcase).mtc{1,mk1})
            for i2=1:length(Lphase(PHcase).mtc{2,mk2})
                samp.freq(mk1,mk2)=samp.freq(mk1,mk2)+SM.paifreq(Lphase(PHcase).mtc{1,mk1}(i1),Lphase(PHcase).mtc{2,mk2}(i2));
            end
        end
    end
end

...
simutime=500;
savename=strcat('FPRp_permu',num2str(samplesize));
...
fprintf('FPRp_permu Strat  Samplesize=%d  Scan:%d---%d  Dens=%d \n\n',samplesize,SetScan.start,SetScan.end,SetScan.dens)
...
%     eval.u=tval.u;eval.sigma2=tval.sigma2;
    eval.g=(ones(1,59)/59)';
  ...
  fprintf('FPRpermu/scan %d/%.1f    Max LR=%.2f\n\n',simu,result1);
   
  FPRpermu(:,simu)=result1;
  save(savename,'FPRpermu','samplesize','tval');
